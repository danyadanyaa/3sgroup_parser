...
class Parser(BaseParser):
    def parse_estate(self, loaded):
        for estate in loaded:
            self.extract_data(estate)

    def parse_parking(self, loaded):
        for estate in loaded:
            self.extract_parking(estate)

    def parse_storeroom(self, loaded):
        for estate in loaded:
            self.extract_storeroom(estate)
    ...
    def load_data(self):
        ...
        all_flats = []
        while True:
            with self.session.get(url, params=params) as req:
                data = req.json()
                flats = data['results']
                for flat in flats:
        ...
        self.parse_estate(all_flats)

    def load_parking(self):
        params = {
            'project': 'talisman-na-rokossovskogo',
            'building': 27,
            'floor_id': 1117,
            'order': '-status,price',
            'limit': 100,
        ...
        url = 'https://3s.group/api/parking/selection_scheme/'
        all_flats = []
        with self.session.get(url, params=params) as req:
            data = req.json()
            flats = data['results'][0]['floor_set'][0]['property_set']
            for flat in flats:
                all_flats.append(flat)
        self.parse_parking(all_flats)

    def load_storeroom(self):
        params = {
            'project': 'talisman-na-rokossovskogo',
            'building': 27,
            'floor_id': 1117,
        ...
        url = 'https://3s.group/api/storage/selection_scheme/'
        all_flats = []
        with self.session.get(url, params=params) as req:
            data = req.json()
            flats = data['results'][0]['floor_set'][0]['property_set']
            for flat in flats:
                all_flats.append(flat)
        self.parse_storeroom(all_flats)

    def get_flat_url(self, link):
        return f"https://3s.group/flats/{link}"

    def extract_data(self, data, head=None):
        obj = EstateObject(self.url_base)

        obj.flat_url = self.get_flat_url(data['id'])
        obj.set_obj_type('flat')
        obj.set_plan(data['plan'])
        obj.set_area(data['area'])
        obj.set_complex(f"{data['project_name']} (Москва)")
        obj.set_floor(data['floor'])
        obj.set_rooms(data['rooms'])
        if data['is_euro']:
            obj.set_feature('Кухня-гостинная')
        obj.set_number(data['number'])
        obj.set_article(data['layout_name'])
        obj.set_building(data['building'])
        if data["with_master_bedroom"]:
            obj.set_feature('Мастер-спальня')
        if data["with_loggia"]:
            obj.set_feature('Наличие лоджии')
        if data["with_higher_ceiling_height"]:
            obj.set_feature('Увеличенная высота потолков')
        if data["with_cornered_placement"]:
            obj.set_feature('Угловая')
        if data["with_good_window_view"]:
            obj.set_feature('Видовая')
        if 'booked' in data['status']:
            obj.set_sale_status('Забронировано')
        if data['discount']:
            if obj.finished:
                obj.set_price_finished(data['original_price'])
                obj.set_price_finished_sale(data['price'])
            else:
                obj.set_price_base(data['original_price'])
                obj.set_price_sale(data['price'])
        else:
            if obj.finished:
                obj.set_price_finished(data['price'])
            else:
                obj.set_price_base(data['price'])
        if data['promo']:
            obj.set_sale(data['promo']['title'])

        self.save_JS_obj(obj)

    def extract_parking(self, data, head=None):
        obj = EstateObject(self.url_base)
        obj.set_obj_type('parking')
        obj.set_area(data['area'])
        obj.set_complex("Талисман на Рокоссовского (Москва)")

        obj.set_floor(data['floor_number'])
        obj.set_number(data['number'])
        obj.set_building(data['building_number'])
        if 'booked' in data['status']:
            obj.set_sale_status('Забронировано')
        obj.set_price_base(data['price'])
    ...
    def extract_storeroom(self, data, head=None):
        obj = EstateObject(self.url_base)
...
def price():
    parser = Parser(url_base='https://3s.group/')
    parser.load_data()
    parser.load_parking()
    parser.load_storeroom()
